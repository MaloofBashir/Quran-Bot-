...
This bot will help the users to easily retrive Quranic ayahs on twitter micro blogging site,The bot has been restricted to fetch only single ayah at once because of twitter limting API request policy

<h3>How to retrieve The ayah</h3>
In its simplified version the English translation of ayah can be retrieved by first mentioning the username of bot (@AlQuranBot_) followed by two paramaters(key value pairs) <mark>surah={value}</mark> or <mark>s_name={value}</mark> and <mark>ayah={value}<mark> e.g <br>
 ...
 <br>
By default the bot will retrieve and reply the mentioned tweet with Englsih Translation by Ahmad Ali,You can change the edition(translation) by passing in the <mark>ed</mark> parameter ,hook with us and and we will discuss how to do that very soon
<br>
